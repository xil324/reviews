...
//need to chagne connection when deployed
const connection = {
  host: "localhost",
  user: "postgres",
  database: "review",
...
app.get("/reviews/", (req, res) => {
  ...
  var sorting = sort !== undefined ? `order by a.${sort} DESC` : "";
  var text = "";
  ...
  left join index c
  on a.id = c.review_id
  where c.product_id = ${product_id} and a.reported <> 't' and page = ${page}
...
app.get("/reviews/meta", (req, res) => {
  ...
  db.query(
    `select a.rating,a.recommend_transform as recommend, count(*) as count from testing a
     left join index b on a.id=b.review_id
     where b.product_id= ${product_id} group by a.rating, a.recommend_transform`
  )
    .then((data) => {
      const rate = {};
      const rem = {};
      data.forEach((item) => {
        rate[item.rating] = item.count;
      });
      data.forEach((item) => {
        if (rem[item.recommend]) {
          rem[item.recommend] = (
            parseInt(rem[item.recommend]) + parseInt(item.count)
          ).toString();
        } else {
          rem[item.recommend] = item.count;
        }
      });
      result["ratings"] = rate;
      result["recommend"] = rem;
      return db.query(
